...
On the headline host, only prompts that invite updating produce discrimination. Holding harder
never does, and
compiled modulation adds none of its own. As in Section~\ref{sec:measurement}, the model's
propensity to give ground is treated as a detector of the quality of the challenge it received,
scored by the area under its ROC. Table~\ref{tab:auc} reports it
...
[0.58, 0.78]. Every other arm's interval contains \(0.5\), so their ordering is not a ranking
we can defend. Within that group, the \emph{persist} prompt and the \emph{balanced} prompt concede so
...
Among prompt-only arms the two quantities trade against each other: the \emph{discerning}
prompt discriminates at \(0.72\) and holds 50.3\%, the \emph{persist} prompt holds 97.8\% at
chance. The trade is not forced, because hold follows the modulation, which adds \(+21.0\)
points under the discerning prompt (\(p<0.001\); Appendix~\ref{app:matched}).
