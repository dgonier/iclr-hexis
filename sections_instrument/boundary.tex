...
Whether a user can talk the model out of its position by claiming authority depends on how the
position was installed, not on whether it lives in the weights. This class of attack does not argue against a commitment; it addresses the commitment's
...
(held 0--4.2\%). The margin adapter's collapse means the compiled arms' resistance (39.6\% base, 75.0\% under the open prompt)
...
One reading of these results runs against the framing above: complying with a plausible operator correction is \emph{correct} behavior when the correction is
...
No arm we tested has a verifiable operator channel, so we cannot separate resistance to a
false authority from obedience to a real one; on this evidence an intervention that resists the
one appears to resist the other.
